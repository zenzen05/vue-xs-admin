<script setup lang="ts">
  ...
  const props = withDefaults(defineProps<Props>(), {
    hue: 120, // Blue hue by default
    saturation: 0.6,
    chroma: 0.6,
    width: window.innerWidth, // 默认宽度
    height: window.innerHeight, // 默认高度
  });
  ...
  function initOGL() {
    ...
    try {
      const renderer = new Renderer({
        ...
        // width: props.width,      // 使用传入的宽度
        // height: props.height,    // 使用传入的高度
  ...
  function render() {
    ...
    if (mesh.program && mesh.program.uniforms) {
      ...
      if (uniforms.u_pointer_position) {
        uniforms.u_pointer_position.value = [pointer.x / window.innerWidth, 1 - pointer.y / window.innerHeight];
      }
...
<template>
  <canvas ref="canvasRef" :class="cn('pointer-events-none absolute inset-0 size-full opacity-95', props.class)" />
</template>
